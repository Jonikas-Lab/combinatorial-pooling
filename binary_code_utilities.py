...
class Binary_code:
    ...
    def get_code_from_generator_matrix(self,generator_file=None,generator_matrix=None,expected_count=0):
        ...
        if not self.length==output_code_length:
            raise BinaryCodeError("Trying to use a generator matrix of shape %s to generate codewords of length %s\
                                  - the sizes don't match up!")%(generator_matrix.shape,self.length)
        if expected_count and not expected_count==codeword_count:
            raise BinaryCodeError("The number of codewords generated by the given matrix will be %s, \
                                  not %s as expected!")%(codeword_count,expected_count)
    ...
    def find_bit_sum_counts(self):
        ...
        return dict(bit_sum_counts)     # convert to a normal dictionary before returning, for cleaner display

    def add_parity_bit(self):
        """ Return a new Binary_code instance generated by adding a parity bit to the current codewords.
        ...
        # TODO implement!  Should return a new Binary_code instance with a length higher by 1, or fail if the minimum distance isn't odd?
        pass

    def filter_by_bit_sum(self,low,high):
